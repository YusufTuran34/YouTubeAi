...
```bash
bash sh_scripts/generate_title.sh
bash sh_scripts/generate_description.sh
```
...
- `bash sh_scripts/run_generation_pipeline.sh` - generate video, description, thumbnail and title sequentially.
- `bash sh_scripts/run_pipeline_and_upload.sh <hours> [options]` - run the pipeline and optionally upload and tweet.
- `bash sh_scripts/run_pipeline_and_stream.sh <hours> [options]` - run the pipeline then stream the result and optionally tweet.
- `bash sh_scripts/generate_gif_with_openai.sh` - create a background GIF with OpenAI (requires API key).
...
```
bash sh_scripts/post_to_twitter.sh
```
...
```bash
bash sh_scripts/post_to_twitter.sh --tag lofi
```
